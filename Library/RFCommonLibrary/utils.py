...
import random
import string
from .py3_mysql import DBReader
from .http import HttpClient
from .codec import *
from .py3_mongodb import MongodDB
import re


class RFLibrary(object):
    def __init__(self):
        self.res = ''
        self.db = DBReader()
        self.http = HttpClient()
        self.mongodb = MongodDB()
        pass
    ...
    def loadMongoDBConfig(self, host, port, user_name, password, db_name, collection_name):
        ...
        :return: 返回集合的信息
        '''
        collection = self.mongodb.loadMongoDBConfig(host, int(port), user_name, password, db_name, collection_name)
        return collection
